...
function MoonCake:start (port, host)
    ...
    if self.isHttps == true then
        local key = fs.readFileSync(self.keyPath.."/key.pem")
        local cert = fs.readFileSync(self.keyPath.."/cert.pem")
        local keyConfig = {
            key = key,
            cert = cert
        }
        ...
        if fs.existsSync(filePath) then
            if fs.statSync(filePath).type == "file" then
                ...
                if options.index then
                    ...
                    print("trimedRoutePath",trimedRoutePath)
