...
local audCheck = {["680656297638-r9vbgsbimrqueto0r5e9v5ulvmjo0fg1.apps.googleusercontent.com"]=true,}
...
do --get latest users at start
	...
	local pattern = "sub=[%s-];table=%s-\n"
	for line in UsersFileRead:lines() do
		local sub,tab = string.gfind(line, pattern)
		if sub and tab then
			Users[sub] = loadstring(TableToLoadstringFormat(tab))()
		end
	end
	UsersFileRead:close()
end
...
function validateToken(token) --https://oauth2.googleapis.com/tokeninfo?id_token=XYZ123
		...
		local res, body = pcall(http.request,"GET",uri)
		if res then
			if not audCheck[body.aud] then return false,"Invalid aud",400 end
			return true,body,200
		else print("ERROR: ",body) return false,body,500
		end
end
...
function module.userAccess(token)
	...
	if sub then
		if Users[sub].token == token then
			...
			Users[sub].token = token
			return true,"Valid new token",200
		end
	else
...
function module.setupServer(server)
	if server then--for debugging
		...
		server:post("/analytics/auth/login", function(req, res)--Website home page
			coroutine.wrap(function()
				local token = req.pargams.token
				if token then
